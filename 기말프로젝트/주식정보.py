...
def save_graph_data_to_csv(stock_name):
    if stock_name == '랜덤':
        # CSV 파일에서 종목 정보를 읽어옴
        stock_codes = pd.read_csv('stock_codes.csv')

        # 랜덤한 주식 선택
        random_index = random.randint(0, len(stock_codes) - 1)
        stock_name = stock_codes.loc[random_index, 'company']

    csv_filename = f'{stock_name}_data.csv'
